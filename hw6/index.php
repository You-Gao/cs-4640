...
class Database {
    ...
    public function checkUser($email) {
        $query = "SELECT * FROM hw6_users WHERE email = $1";
        $res = $this->query($query, $email);
        return count($res) > 0;
    }

    public function checkPassword($email, $password) {
        $query = "SELECT password FROM hw6_users WHERE email = $1";
        $res = $this->query($query, $email);
        return password_verify($password, $res[0]["password"]);
    }

    public function getUserID($email) {
        $query = "SELECT id FROM hw6_users WHERE email = $1";
        $res = $this->query($query, $email);
        return $res[0]["id"];
    }

    public function storeUserWords($user_id, $word) {
        $query = "INSERT INTO hw6_userwords (user_id, word_id) VALUES ($1, $2)";
        $res = $this->query($query, $user_id, $word);
    }

    public function createUser($name, $user_name, $email, $password) {
        $query = "INSERT INTO hw6_users (name, email, password, display, score) VALUES ($1, $2, $3, $4, $5)";
...
class AnagramsGameController {
    ...
    private function saveScores(){
        ...
        $this->db->query("INSERT INTO hw6_userstats (user_id, stat_id, word_id) VALUES ($1, $2, $3)", $_SESSION["user_id"], $_SESSION["score_id"], $_SESSION["word_id"]);
    }
    ...
    private function handleGameOver() {
        ...
        $query = "SELECT * FROM hw6_userstats LEFT JOIN stats ON hw6_userstats.stat_id = hw6_stats.id LEFT JOIN hw6_word ON hw6_userstats.word_id = hw6_word.id WHERE hw6_userstats.user_id = $1";
    ...
    private function setWordsRemaining() {
        ...
        $this->db->query("UPDATE hw6_stats SET words_remaining = $1 WHERE id = $2", $words_remaining, $_SESSION["score_id"]);
    }
    ...
    private function setupGame() {
        ...
        $this->db->query("INSERT INTO hw6_stats (score) VALUES (0)");
        // grabs the most recent score id
        $_SESSION["score_id"] = $this->db->query("SELECT id FROM hw6_stats ORDER BY id DESC LIMIT 1")[0]["id"];

        $words = file($words7_path);
        $existing_words = $this->db->query("SELECT word_id FROM hw6_userwords WHERE user_id = $1", $_SESSION["user_id"]);
        ...
        $this->db->query("INSERT INTO hw6_word (word) VALUES ($1)", $word);
        $this->db->storeUserWords($_SESSION["user_id"], $this->db->query("SELECT id FROM hw6_word WHERE word = $1", $word)[0]["id"]);
        $_SESSION["word"] = strtoupper(trim($word));
        $_SESSION["word_id"] = $this->db->query("SELECT id FROM hw6_word WHERE word = $1", $word)[0]["id"];
    ...
    private function checkGuess($guess) {
        ...
        if (in_array($guess, $word_bank[strlen($guess)])) {
            ...
            $this->db->query("UPDATE hw6_stats SET score = $1 WHERE id = $2", $_SESSION["score"], $_SESSION["score_id"]);
